    ...
    public partial class MainWindow : MetroWindow
    {
        ...
        private void LoadDataWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            if (Lang.Character.Quests != null)
            {
                ...
                List<QuestData> qData = JsonConvert.DeserializeObject<QuestsData>(File.ReadAllText(Path.Combine(Lang.options.EftServerPath, "user", "cache", "quests.json"))).data;
            ...
            if (Lang.Character.Hideout != null)
            {
                ...
                var areas = JsonConvert.DeserializeObject<HideoutData>(File.ReadAllText(Path.Combine(Lang.options.EftServerPath, "user", "cache", "hideout_areas.json"))).data;
            ...
            if (Lang.Character.Suits != null)
            {
                ...
                foreach (var s in Directory.GetDirectories(Path.Combine(Lang.options.EftServerPath, "db", "cacheBase", "traders")).Where(x => File.Exists(Path.Combine(x, "suits.json"))))
                {
        ...
        private void Window_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            if (string.IsNullOrWhiteSpace(Lang.options.Language) || string.IsNullOrWhiteSpace(Lang.options.EftServerPath)
                ...
                || string.IsNullOrWhiteSpace(Lang.options.DefaultProfile) || !File.Exists(Path.Combine(Lang.options.EftServerPath, "user\\profiles", Lang.options.DefaultProfile, "character.json")))
        ...
        private void SaveProfileWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            var Stash = jobject.SelectToken("Inventory").SelectToken("items").ToObject<Character.Character_Inventory.Character_Inventory_Item[]>();
            ...
            for (int index = 0; index < Stash.Count(); index++)
            {
                ...
                {
                    Dispatcher.Invoke(() => jobject.SelectToken("Inventory").SelectToken("items")[index]["_tpl"] = BigPocketsSwitcher.IsOn ? "5af99e9186f7747c447120b8" : "557ffd194bdc2d28148b457f");
                    continue;
                }
                if (!iDs.Contains(probe.Id))
                {
                    foreach (var obj in jobject.SelectToken("Inventory").SelectToken("items").ToList())
                    {
            ...
            foreach (var item in Lang.Character.Inventory.Items.Where(x => !Stash.Select(b => b.Id).ToList().Contains(x.Id)))
                jobject.SelectToken("Inventory").SelectToken("items").LastOrDefault().AddAfterSelf(ExtMethods.RemoveNullAndEmptyProperties(JObject.FromObject(item)));
            ...
            for (int index = 0; index < Lang.Character.Hideout.Areas.Count(); ++index)
            {
                ...
                var startLevel = Int32.Parse(jobject.SelectToken("Hideout").SelectToken("Areas")[index]["level"].ToString());
                jobject.SelectToken("Hideout").SelectToken("Areas")[index]["level"] = probeLevel;
                                    ...
                                    if (bonus != null)
                                    {
                                        switch (bonus.Type)
                                        {
                                            ...
                                            case "MaximumEnergyReserve":
                                                jobject.SelectToken("Health").SelectToken("Energy")["Maximum"] = 110;
                                                break;
                                            default:
                                                break;
                                        }
                                        jobject.SelectToken("Bonuses").LastOrDefault().AddAfterSelf(JObject.FromObject(t));
                                    }
                                }
                            }
                        }
            ...
            foreach (var tr in Lang.Character.TraderStandings)
            {
                ...
                jobject.SelectToken("TraderStandings").SelectToken(tr.Key)["display"] = Lang.Character.TraderStandings[tr.Key].Display;
            }
            if (Lang.Character.Quests.Count() > 0)
            {
                var questsObject = jobject.SelectToken("Quests").ToObject<Character.Character_Quests[]>();
                for (int i = 0; i < questsObject.Count(); i++)
                {
                    var quest = jobject.SelectToken("Quests")[i].ToObject<Character.Character_Quests>();
                    if (quest != null)
                        jobject.SelectToken("Quests")[i]["status"] = Lang.Character.Quests.Where(x => x.Qid == quest.Qid).FirstOrDefault().Status;
                }
                if (questsObject.Count() > 0)
                {
                    foreach (var quest in Lang.Character.Quests)
                    {
                        if (questsObject.Where(x => x.Qid == quest.Qid).Count() < 1)
                            jobject.SelectToken("Quests").LastOrDefault().AddAfterSelf(JObject.FromObject(quest));
                    }
                }
                else
                    jobject.SelectToken("Quests").Replace(JToken.FromObject(Lang.Character.Quests));
            }
            if (Lang.Character.Skills.Mastering.Count() > 0)
            {
                var masteringObject = jobject.SelectToken("Skills").SelectToken("Mastering").ToObject<Character.Character_Skills.Character_Skill[]>();
                for (int i = 0; i < masteringObject.Count(); i++)
                {
                    var probe = jobject.SelectToken("Skills").SelectToken("Mastering")[i].ToObject<Character.Character_Skills.Character_Skill>();
                    if (probe != null)
                        jobject.SelectToken("Skills").SelectToken("Mastering")[i]["Progress"] = Lang.Character.Skills.Mastering.Where(x => x.Id == probe.Id).FirstOrDefault().Progress;
                }
                if (masteringObject.Count() > 0)
                {
                    foreach (var master in Lang.Character.Skills.Mastering)
                    {
                        if (masteringObject.Where(x => x.Id == master.Id).Count() < 1)
                            jobject.SelectToken("Skills").SelectToken("Mastering").LastOrDefault().AddAfterSelf(JObject.FromObject(master));
                    }
                }
                else
            ...
            void ChangeStash(string tpl)
            {
                if (!string.IsNullOrEmpty(tpl))
                {
                    for (int index = 0; index < Stash.Count(); index++)
                    {
                        ...
                        {
                            jobject.SelectToken("Inventory").SelectToken("items")[index]["_tpl"] = tpl;
                        }
                    }
                }
            }
