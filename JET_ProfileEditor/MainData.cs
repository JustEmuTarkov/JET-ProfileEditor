...
namespace JET_ProfileEditor
{
    public class MainData
    {
        ...
        public static MainData Load()
        {
            ...
            if (!string.IsNullOrEmpty(eOptions.DefaultProfile) && !string.IsNullOrEmpty(eOptions.EftServerPath) && !File.Exists(Path.Combine(eOptions.EftServerPath, "user\\profiles", eOptions.DefaultProfile, "character.json")))
            ...
            {
                lang.Profiles = Directory.GetDirectories(eOptions.EftServerPath + "\\user\\profiles").Select(x => Path.GetFileName(x)).ToList();
            ...
            {
                var Pr = JsonConvert.DeserializeObject<Character>(File.ReadAllText(Path.Combine(eOptions.EftServerPath, "user\\profiles", eOptions.DefaultProfile, "character.json")));
                lang.Character = Pr;
                var storage = JsonConvert.DeserializeObject<Storage>(File.ReadAllText(Path.Combine(eOptions.EftServerPath, "user\\profiles", eOptions.DefaultProfile, "storage.json")));
                // get user/profiles/ID/storage.data.suites to get the profile suites
                if(storage.data.Suites.Length > 0)
                    lang.Character.Suits = storage.data.Suites.ToList();
        ...
        static Dictionary<string, string> EN => new Dictionary<string, string>()
        {
            ...
            ["server_select"] = "Select the JET Server directory.",
            ["invalid_server_location_caption"] = "Error",
            ["invalid_server_location_text"] = "The selected path does not seem to be a JET server location.",
            ...
            ["tab_settings_server"] = "JET Server directory",
            ...
            ["tab_about_text"] = "Program for editing player profile on the JET server",
        ...
        static Dictionary<string, string> GE => new Dictionary<string, string>
        {
            ...
            ["server_select"] = "Wählen Sie das JET Server-Verzeichnis aus.",
            ["invalid_server_location_caption"] = "Error",
            ["invalid_server_location_text"] = "Der ausgewählte Pfad scheint kein JET-Serverstandort zu sein.",
            ...
            ["tab_settings_server"] = "JET Server verzeichnis",
            ...
            ["tab_about_text"] = "Programm zum Bearbeiten des Spielerprofils auf dem JET-Server",
        ...
        static Dictionary<string, string> RU => new Dictionary<string, string>
        {
            ...
            ["server_select"] = "Выберите папку с сервером JET.",
            ["invalid_server_location_caption"] = "Ошибка",
            ["invalid_server_location_text"] = "Сервер JET не найден. Попробовать снова?",
            ...
            ["tab_settings_server"] = "Каталог JET Server",
            ...
            ["tab_about_text"] = "Программа для редактирования профиля игрока на сервере JET",
        ...
        static Dictionary<string, string> FR => new Dictionary<string, string>
        {
            ...
            ["server_select"] = "Sélectionnez le répertoire du serveur JET.",
            ["invalid_server_location_caption"] = "Erreur",
            ["invalid_server_location_text"] = "Le chemin sélectionné ne semble pas être un emplacement de serveur JET. Réessayer?",
            ...
            ["tab_settings_server"] = "Répertoire du serveur JET",
            ...
            ["tab_about_title"] = "Sur",
            ["tab_about_text"] = "Programme d'édition du profil du joueur sur le serveur JET",
